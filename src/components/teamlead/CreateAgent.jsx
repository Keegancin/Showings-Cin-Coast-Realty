...
const CreateAgent = () => {
  ...
  const { user, createAgent } = useAuthStore();
  ...
  const handleSubmit = (e) => {
    ...
    const result = createAgent(formData);

    if (result.success) {
      navigate('/team-lead/team');
    } else {
  ...
  return (
    <div className="px-4 pt-6 pb-4">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h2 className="font-display text-2xl font-semibold text-white">Add Team Member</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          <label htmlFor="title" className="block text-xs uppercase tracking-wider text-brand-text mb-2">
            Title *
          </label>
          <input
            id="title"
        ...
        <div>
          <label htmlFor="bio" className="block text-xs uppercase tracking-wider text-brand-text mb-2">
            Bio
          </label>
          <textarea
            id="bio"
            ...
            rows="3"
            placeholder="Tell clients about this agent..."
          />
        </div>
        ...
        <div>
          ...
          <input
            ...
            required
          />
          <p className="text-xs text-brand-text mt-1">Agent can change this after first login</p>
        </div>
